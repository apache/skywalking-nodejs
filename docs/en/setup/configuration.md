...
| `SW_AGENT_COLLECTOR_BACKEND_SERVICES` | `collectorAddress` | `127.0.0.1:11800` | OAP gRPC address(es). One `host:port` uses grpc-js `dns:` (all A/AAAA become endpoints; re-resolved on demand by grpc-js, not on a fixed agent timer). A comma-separated list uses a static resolver with `pick_first`. By default each list entry is a literal endpoint (no per-name DNS expansion or re-resolution). Set `SW_AGENT_IS_RESOLVE_DNS_PERIODICALLY=true` to expand every hostname to its A/AAAA records and re-resolve every 30s (override with `SW_AGENT_DNS_RE_RESOLVE_INTERVAL_SECONDS`), rebuilding the channel when the address set changes or retrying open when no channel is installed yet. Failed names keep last-known endpoints in the dial set so partial DNS outages do not shrink a working channel or block recovery when another name's IPs change. Endpoint pick order is shuffled by grpc-js (`shuffleAddressList`); the target string keeps config order so TLS authority / SNI stay on the first list entry (after DNS expand, the first configured hostname in the list is used as authority when dialing IPs). Under TLS, all backends must present certificates that share the needed SANs. Prefer one DNS name with multiple A/AAAA records for TLS high availability when periodic multi-name DNS is not enabled. |
| `SW_AGENT_IS_RESOLVE_DNS_PERIODICALLY` | `isResolveDnsPeriodically` | `false` | When `true` and `collectorAddress` lists multiple backends that include hostnames, expand each name to all A/AAAA addresses (per-name lookup timeout 5s) and periodically re-resolve (30s). Rebuilds when the resolved set changes; also retries open on later ticks if the channel was never opened. Single-address targets ignore this flag (grpc-js `dns:` already re-resolves). All-IP multi-address lists do not start a timer. `SW_AGENT_COLLECTOR_IS_RESOLVE_DNS_PERIODICALLY` is accepted as a Java-aligned alias. |
| `SW_AGENT_DNS_RE_RESOLVE_INTERVAL_SECONDS` | — | `30` | Interval for multi-backend DNS re-resolve when `SW_AGENT_IS_RESOLVE_DNS_PERIODICALLY=true`. Positive decimal integer seconds for the whole value (rejects 1.5, 1e3, 10junk); invalid values and values that overflow the Node timer limit fall back to 30. `SW_AGENT_COLLECTOR_GRPC_CHANNEL_CHECK_INTERVAL` is accepted as a Java-aligned alias (same default 30s). Overlapping ticks while a resolve is still running are skipped. |
